...
use jevons_engine::{
    Error, Segment, SpeechConfig, SpeechInfo, Transcriber, Transcript, Transcription, Word, words,
};
...
#[cfg(test)]
impl jevons_engine::SpeechModel for ScriptedModel {
    ...
    fn transcribe(
        &mut self,
        samples: &[f32],
    ) -> jevons_engine::Result<Vec<jevons_engine::SpeechToken>> {
        let level = |x: f32| {
            if x >= 0.005 {
        ...
        for end in 1..=samples.len() {
            ...
            if n > 0 && end - start >= 50 {
                ...
                tokens.push(jevons_engine::SpeechToken {
                    id: n,
    ...
    fn detokenize(&self, ids: &[u32]) -> jevons_engine::Result<String> {
        Ok(ids
            .iter()
