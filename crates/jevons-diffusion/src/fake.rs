//! A scripted in-memory model for engine and service tests without a GPU or model files.
use jevons_core::{
...
/// An engine on `model`, and the log of everything it asks of the model.
pub fn fake_engine(model: FakeModel) -> (crate::DiffusionEngine, Rc<RefCell<Log>>) {
    let log = model.log.clone();
    (crate::DiffusionEngine::new(Box::new(model)).unwrap(), log)
}

/// A masked-diffusion fake: blocks of 4, commit threshold 0.9, at most 4 steps.
pub fn masked_model() -> FakeModel {
...
/// Tokens of `text` under the fake tokenizer.
pub fn tokens(text: &str, bos: bool, special: bool) -> Vec<i32> {
