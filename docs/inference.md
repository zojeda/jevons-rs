...
- **`think`.** The model is trained with causal and diffusion objectives on one set of weights, so the same head predicts the next token under causal attention. The server's `--think-decoding` picks how thoughts are generated; all three are greedy:
